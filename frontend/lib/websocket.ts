...
class WebSocketManager {
  ...
  private constructor() {
    const apiUrl = typeof window !== 'undefined'
      ? (window.location.hostname.includes('phala.network')
        ? 'https://7cdb6ebb4d8688ed3c25b241bc6e76d9e2b16241-8000.dstack-prod7.phala.network'
        : 'http://localhost:8000')
  ...
  connect(): void {
    ...
    try {
      ...
      this.ws.onopen = () => {
        console.log('Connected to Zobeide WebSocket');
      ...
      this.ws.onclose = () => {
        console.log('Disconnected from Zobeide WebSocket');
