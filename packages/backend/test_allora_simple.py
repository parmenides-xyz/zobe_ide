#!/usr/bin/env python3
"""Test Allora with exact example from docs"""
...
# Example usage - try topic 1 first as in the docs
try:
    data = get_latest_inference(1, "https://allora-rpc.testnet.allora.network/")
    print(f"Topic 1 - Latest inference: {data['network_inferences']['combined_value']}")
    print(f"Confidence intervals: {data['confidence_interval_values']}")
except Exception as e:
    print(f"Failed to get inference for topic 1: {e}")

# Try topic 10 for memecoins
try:
    data = get_latest_inference(10, "https://allora-rpc.testnet.allora.network/")
    print(f"Topic 10 - Latest inference: {data['network_inferences']['combined_value']}")
    print(f"Confidence intervals: {data['confidence_interval_values']}")
except Exception as e:
    print(f"Failed to get inference for topic 10: {e}")