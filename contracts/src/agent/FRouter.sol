    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Graduates the pool by transferring balances to the sender so that they can be deposited into a Uniswap pool.
    // It transfers all the VIRTUAL but only enough token so that the price in the Uniswap pool will continue to be the same.
