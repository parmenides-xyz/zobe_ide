...
"""
Backend API
Middleware for interacting with Kurtosis
"""
...
from src.traders.allora_personalities import get_trader_personality, TRADER_PERSONALITIES
...
app = FastAPI(
    title="Kurtosis API",
    description="API for managing agentic futarchy on Sei",
    version="1.0.0"
)
    ...
    try:
        async def create_market_task():
            try:
                ...
                w3 = Web3(Web3.HTTPProvider(os.getenv("SEI_RPC_URL", "https://evm-rpc-testnet.sei-apis.com")))
                account = Account.from_key(os.getenv("MASTER_PRIVATE_KEY"))
                ...
                # Extract market ID from logs - PROPERLY!
                if receipt['logs']:
                ...
                market_file = Path(__file__).parent.parent / "orchestrator" / "latest_market.txt"
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_text(json.dumps({
            "type": "connected",
            "message": "Connected to Kurtosis WebSocket",
            "timestamp": datetime.now().isoformat()
        }))
...
async def startup_event():
    ...
    logger.info("Kurtosis API starting up...")
...
async def shutdown_event():
    ...
    global broadcast_task
    logger.info("Kurtosis API shutting down...")
