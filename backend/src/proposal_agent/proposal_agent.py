...
import os
from .agent_templates import HARDCODED_PROPOSALS
...
class ProposalAgent:
    ...
    def generate_proposals_via_llm(cls, num_proposals=10):
        ...
        prompt = f"""Generate {num_proposals} unique AI agent proposals for Virtual protocol prediction markets.
...
- capabilities: Array of 2-3 GAME plugin functions (e.g., allora.get_price_inference, cdp.trade, twitter.search_tweets, telegram.send_message, bittensor.call_subnet, imagegen.generate)
- strategy: Brief description of operational strategy

Available GAME plugins:
- Allora: Price predictions and market inferences
- CDP: Coinbase trading and wallet management on Base
- Bittensor: AI content detection
- Twitter: Social sentiment analysis
- Telegram: Alert and signal aggregation
- ImageGen: NFT art generation

Focus on agents that combine multiple plugins for unique value propositions.
...
JSON array:"""
    ...
    def get_agent_proposals(cls):
        ...
        print("Using hardcoded proposals as fallback")
        return HARDCODED_PROPOSALS
